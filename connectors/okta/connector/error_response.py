...
class ErrorResponder:
    ...
    @staticmethod
    def is_plain_string(res):
        """ Checks value is string or not """
        return isinstance(res, str) and not res.startswith('<?') and not res.startswith('{')

    @staticmethod
    def is_json_string(res):
        """ Checks value is json string or not """
        return isinstance(res, str) and res.startswith('{')
